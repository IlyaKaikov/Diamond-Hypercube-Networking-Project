...
def parse_args():
    parser = argparse.ArgumentParser(description=("Runs flows under random-uniform background traffic, and outputs FCT results to csv file."))
    parser.add_argument("--topo", required=True, choices=["mesh", "torus2d", "dq"],)
    parser.add_argument("--r", type=int,)
    parser.add_argument("--d", type=int,)
    parser.add_argument("--bg-num-flows", type=int, default=None,) # Number of background flows (random src/dst pairs). Overrides --bg-multiplier
    parser.add_argument("--bg-multiplier", type=int, default=1,) # Flows per host (k). Ignored if --bg-num-flows is set.
    parser.add_argument("--bg-parallel-streams",type=int, default=1,) # iperf3 -P value for background flows (parallel streams per flow)
    ...
    parser.add_argument("--probe-megabytes", type=float, default=10.0,)
    parser.add_argument("--probe-port", type=int, default=4000,)
...
def main():
    ...
    try:
        ...
        info(f"**** Generating random-uniform pairs (seed={args.seed}) ****\n")
        ...
        info(f"**** Starting {bg_num_flows} background flows ****\n")
